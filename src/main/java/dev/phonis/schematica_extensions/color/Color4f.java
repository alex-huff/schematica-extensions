package dev.phonis.schematica_extensions.color;

import dev.phonis.schematica_extensions.util.MathUtil;
...
public class Color4f
{
    ...
    {
        return String.format("Color4f{hex=%s, a = %f, r = %f, g = %f, b = %f, intValue = %d}", getHexColorString(this.intValue), this.a, this.r, this.g, this.b, this.intValue);
    }
    ...
    {
        return getColorFromString(str, Color4f.WHITE);
    }
     ...
     */
    public static Color4f getColorFromString(String colorStr, Color4f defaultColor)
    {
        ...
        if (matcher.matches())
        {
            try
            {
                return Color4f.fromColor((int) Long.parseLong(matcher.group(1), 16));
            }
        ...
        if (matcher.matches())
        {
            try
            {
                return Color4f.fromColor(0xFF000000 | (int) Long.parseLong(matcher.group(1), 16));
            }
            catch (NumberFormatException ignore)
            {
                return defaultColor;
        ...
        if (matcher.matches())
        {
            try
            {
                ...
                return Color4f.fromColor(a << 24 | r << 16 | g << 8 | b);
            }
            catch (NumberFormatException ignore)
            {
            }
        ...
        if (matcher.matches())
        {
            try
            {
                ...
                int g = ((orig & 0x00F0) >>> 4) * 17;
                int b = ((orig & 0x000F)) * 17;
                return Color4f.fromColor(0xFF000000 | r << 16 | g << 8 | b);
            }
            catch (NumberFormatException ignore)
            {
            }
        ...
        try
        {
            return Color4f.fromColor(Integer.parseInt(colorStr, 10));
        }
